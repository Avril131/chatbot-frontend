...
export default function Navbar(
    { hasSidebarIcon, openSidebar, unlogin, setUnlogin }:
        {
            hasSidebarIcon: boolean;
            openSidebar: () => void;
            unlogin: boolean;
            setUnlogin: (arg0: boolean) => void;
        }
) {
    ...
    const handleLoginSuccess = async (response: any) => {
        ...
        try {
            const response = await instance.post("/auth/google", { token: authorizationCode })
    ...
    const getUserInfo = async () => {
        try {
            const response = await instance.post("/auth/info")
    ...
    useEffect(() => {
        ...
        if (!unlogin && avatar === "") {
            setAvatar(JSON.parse(localStorage.getItem("userInfo") || "{}")?.picture || "")
        }
    }, [])

    return (
        <div className="navbar">
            {hasSidebarIcon && !unlogin &&
                <>
                    <Tooltip
                        ref={tooltipRef}
            ...
            {unlogin ?
                <GoogleOAuthProvider clientId="558505161169-ii37bd6fbohcinaddjevrnd6rasomud2.apps.googleusercontent.com">
